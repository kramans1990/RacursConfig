...
namespace RacursConfig.PagesVM
{
    class SatellitePageVM : BaseVM
    {
        ...
        public RelayCommand OpenSattelliteModelEdtorCommand { get; set; }
        public RelayCommand OpenSattelliteModesEdtorCommand{get;set;}
        public SatellitePageVM()
        {
            ...
            OpenSattelliteModesEdtorCommand = new RelayCommand(x => openModesEditor());
        ...
        private void OpenModelEditor()
        {
            ...
            Satellite model = JsonSerializer.Deserialize<Satellite>(JsonConvert.SerializeObject(SatelliteEditor), options);
            SatellitteComponetsEditor editorWindow = new SatellitteComponetsEditor(SatelliteEditor);
            if (editorWindow.ShowDialog() != true)
            {
                SatelliteEditor = model;
                setComponentsToSlots();
            }
        ...
        private void openModesEditor()
        {
            ...
            Satellite model = JsonSerializer.Deserialize<Satellite>(JsonConvert.SerializeObject(SatelliteEditor), options);
            SatelliteModesEditor editorWindow = new SatelliteModesEditor(SatelliteEditor);
            if (editorWindow.ShowDialog() != true)
            {
                SatelliteEditor.Modes = editorWindow.Modes;
                setComponentsToSlots();
            }
            SatelliteEditor.Modes = editorWindow.Modes;
        }
        ...
        private async void getSatelliteModel(int satId)
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    SatelliteEditor = JsonSerializer.Deserialize<Satellite>(result.Result, options);  //setComponentsToSlots();
        ...
        private void setComponentsToSlots()
        {
        ...
        {
            EditorVisibility = Visibility.Visible;
            SatelliteEditor = new Satellite {Name="Name" , SatelliteType="A"};
            mode = "Add";
            setComponents();  
        ...
        private async void EditSatellite()
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    Messages.Add(GetTimeLabel() + response.ReasonPhrase);
                }
            }
            catch (Exception exception)
            {
